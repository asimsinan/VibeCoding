...
-- 
-- This migration creates the complete database schema for the Personal Shopping Assistant
...
CREATE TABLE products (
    ...
    CONSTRAINT products_style_check CHECK (style IS NULL OR LENGTH(style) <= 100),
...
CREATE INDEX idx_user_preferences_brands ON user_preferences USING GIN(brands);
CREATE INDEX idx_user_preferences_price_range ON user_preferences(price_range_min, price_range_max);
...
CREATE INDEX idx_interactions_product_type ON interactions(product_id, type);
...
CREATE INDEX idx_products_brand_availability ON products(brand, availability);
CREATE INDEX idx_products_category_availability ON products(category, availability);
CREATE INDEX idx_products_price_availability ON products(price, availability);
...
CREATE INDEX idx_recommendation_performance_algorithm ON recommendation_performance(algorithm);
...
DROP MATERIALIZED VIEW IF EXISTS recommendation_performance CASCADE;

-- Drop functions
DROP FUNCTION IF EXISTS refresh_recommendation_performance() CASCADE;
DROP FUNCTION IF EXISTS cleanup_expired_recommendations() CASCADE;
DROP FUNCTION IF EXISTS get_product_popularity_score(INTEGER) CASCADE;
DROP FUNCTION IF EXISTS get_user_interaction_stats(INTEGER) CASCADE;

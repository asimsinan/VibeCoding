...
import { CertificatesList, Alert, AlertDescription, LoadingSpinner, Button } from '@/components/ui';
import { useSession, signOut } from 'next-auth/react';
import Link from 'next/link';
import { apiClient, ApiError } from '@/lib/api';
import { Certificate } from '@/components/ui/certificates';
...
export default function CertificatesPage() {
  ...
  useEffect(() => {
    const fetchCertificates = async () => {
      ...
      try {
        setLoading(true);
        const certificatesData = await apiClient.get<Certificate[]>('/api/student/certificates');
  ...
  const handleDownload = (certificate: Certificate) => {
    window.open(`/api/student/certificates/${certificate.id}/download`, '_blank');
  };

  const handleVerify = (certificate: Certificate) => {
    window.open(`/certificates/verify?certificateNumber=${certificate.certificateNumber}`, '_blank');
  };
      ...
      <div className="flex h-screen">
        ...
        <div className="p-6">
          ...
          <nav className="space-y-3">
            ...
            <Link href="/student/certificates" className="flex items-center px-4 py-3 text-sm font-medium text-red-600 bg-red-50 rounded-xl group">
              <svg className="w-5 h-5 mr-3 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">

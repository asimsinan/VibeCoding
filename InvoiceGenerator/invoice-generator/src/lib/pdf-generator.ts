 ...
 * 
 * This service handles the generation of PDF invoices using jsPDF library.
 * It provides functions for creating, styling, and downloading PDF documents.
 */
...
export class PDFGenerator {
  private doc: jsPDF;
  private stylingOptions: PDFStylingOptions;
  private layoutOptions: PDFLayoutOptions;

  constructor(stylingOptions: PDFStylingOptions = {}, layoutOptions: PDFLayoutOptions = DEFAULT_LAYOUT_OPTIONS) {
    this.doc = new jsPDF();
   ...
   */
  private createHeader(invoice: Invoice): void {
   ...
   */
  private createClientInfo(invoice: Invoice): void {
   ...
   */
  private createLineItemsTable(invoice: Invoice): void {
    const { pageMargins } = this.layoutOptions;
   ...
   */
  private createTotalsSection(invoice: Invoice): void {
    const { pageMargins } = this.layoutOptions;
...
export class PDFStyler {
   ...
   */
  static applyStyling(doc: jsPDF, options: PDFStylingOptions = {}): void {
    ...
    doc.setTextColor(parseInt(stylingOptions.primaryColor!.substring(1, 3), 16), 
                     parseInt(stylingOptions.primaryColor!.substring(3, 5), 16), 
                     parseInt(stylingOptions.primaryColor!.substring(5, 7), 16));
  }

  /**
   * Apply professional header styling
   */
  static applyHeaderStyling(doc: jsPDF, options: PDFStylingOptions = {}): void {
    const stylingOptions = { ...DEFAULT_STYLING_OPTIONS, ...options };
   ...
   * Apply table styling
   */
  static applyTableStyling(doc: jsPDF, startX: number, startY: number, width: number, height: number): void {
    // Table border
...
export class PDFDownloader {
   ...
   */
  static async downloadPDF(pdfData: Uint8Array, filename: string): Promise<void> {
    try {
   ...
   */
  static generateFilename(invoiceId: string, date: Date): string {
   ...
   */
  static generateFilenameFromClient(clientName: string, date: Date): string {
 ...
 */
export async function generateAndDownloadPDF(invoice: Invoice, options: PDFStylingOptions = {}): Promise<void> {
  try {
 ...
 */
export async function generatePDF(invoice: Invoice, options: PDFStylingOptions = {}): Promise<Uint8Array> {
  try {
    const generator = new PDFGenerator(options);
 ...
 */
export async function downloadPDF(pdfData: Uint8Array, filename: string): Promise<void> {
  try {
    await PDFDownloader.downloadPDF(pdfData, filename);
  } catch (error) {
 ...
 */
export function generateFilename(invoiceId: string, date: Date): string {

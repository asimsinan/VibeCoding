...
export class DeleteController {
  async deleteUpload(request: NextRequest): Promise<NextResponse> {
    try {
      ...
      if (feedback) {
        try {
          await feedbackModel.deleteByUploadId(uploadId);
        } catch (error) {
      ...
      const message = feedback 
        ? 'Upload and associated feedback deleted successfully'

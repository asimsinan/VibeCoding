...
function getStripe(secretValue?: string): Stripe {
  ...
  if (!stripeInstances.has(secret)) {
    stripeInstances.set(secret, new Stripe(secret, {
  ...
  async (request) => {
    ...
    try {
      ...
      if (!secret) {
        throw new HttpsError(
          "internal",
          "Stripe secret key not available. Please contact support."
        );
      }
      ...
      const paymentIntent = await getStripe(secret).paymentIntents.create(
        paymentIntentParams
      );
...
export const confirmPayment = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      const paymentIntent = await getStripe(secret).paymentIntents.retrieve(
        paymentIntentId
      );
...
export const createSetupIntent = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      throw new HttpsError(
        "internal",
        `Failed to create setup intent: ${errorMessage}`
      );
    }
  }
);
 ...
 */
export const createPaymentMethod = onCall(
  {
    invoker: "public",
    secrets: [stripeSecretKey],
  },
  async (request) => {
    ...
    let paymentMethodIdToUse: string;
    ...
    if (setupIntentId) {
      try {
        ...
        if (!setupIntent.payment_method) {
          throw new HttpsError(
            "invalid-argument",
            "Setup intent does not have a payment method"
          );
        }
        paymentMethodIdToUse = setupIntent.payment_method as string;
      } catch (error: any) {
        throw new HttpsError(
          "internal",
          error.message || "Failed to retrieve setup intent"
        );
      }
      ...
      throw new HttpsError(
        "invalid-argument",
        "Payment method ID or setup intent ID is required"
      );
    }
 ...
 */
async function getOrCreateStripeCustomer(userId: string, secret?: string): Promise<string> {
